...
static int goodix_tool_show(struct seq_file *seq, void *offset);

static int
goodix_tool_proc_open_fs(struct inode *inode, struct file *file)
{
	return single_open(file, goodix_tool_show, inode->i_private);
}
static const struct proc_ops gtp_proc_ops = {
	.proc_open	= goodix_tool_proc_open_fs,
	.proc_read = seq_read,
...
static int goodix_tool_show(struct seq_file *seq, void *offset)
{
	...
	} else if (!cmd_head.wr)
	{
		...
		while (data_len > 0)
		{
			...
			ret = seq_write(seq, &cmd_head.data[GTP_ADDR_LENGTH], len);
			if (ret < 0)
				return ret;
			loc += len;
			addr += len;
	...
	{
		ret = seq_write(seq, IC_TYPE, sizeof(IC_TYPE));
		return ret;
	}
#ifdef CONFIG_TOUCHSCREEN_GT9XX_UPDATE
	else if (cmd_head.wr == 4)
	{
		...
		ret = seq_write(seq, progress_buf, 4);
		return ret;
	}
#endif
	...
	{ /*Read driver version*/
		ret = seq_write(seq, GTP_DRIVER_VERSION, strlen(GTP_DRIVER_VERSION));
		return ret;
	}
