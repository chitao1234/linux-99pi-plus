...
static int gtp_config_seq_show(struct seq_file *seq, void *offset)
{
	int i, ret;
	...
	seq_printf(seq, "====init value====\n");
	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++)
	{
		seq_printf(seq, "0x%02X ", cfg->data[i + 2]);
	...
	seq_printf(seq, "====real value====\n");
	ret = gtp_i2c_read(i2c_connect_client, temp_data,
			GTP_CONFIG_MAX_LENGTH + 2);
	if (ret < 0)
	{
		seq_printf(seq, "Failed read real config data\n");
	} else
	{
		for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++)
		{
			seq_printf(seq, "0x%02X ", temp_data[i + 2]);
...
static int
gtp_config_proc_open_fs(struct inode *inode, struct file *file)
{
	return single_open(file, gtp_config_seq_show, inode->i_private);
}
static const struct proc_ops config_proc_ops = {
	.proc_open	= gtp_config_proc_open_fs,
	.proc_read	= seq_read,
...
static int ls7a_set_gpio_irq_type(struct loongson_gpio_chip *lgpio, unsigned int pin, int flags)
{
	switch (flags)
	{
...
static int ls7a_get_gpio_irq_status(struct loongson_gpio_chip *lgpio, unsigned int pin)
{
	return readb(LS7A_GPIO_INTSTAT_BYTE(lgpio, pin));
}

static int ls7a_clear_gpio_irq_status(struct loongson_gpio_chip *lgpio, unsigned int pin)
{
...
static int ls2_set_gpio_irq_type(struct ls2_gpio_chip *lgpio, unsigned int pin, int flags)
{
	uint32_t u;
...
static int ls7a_set_gpio_inten(struct loongson_gpio_chip *lgpio, unsigned int pin, int enable)
{
	writeb(enable, LS7A_GPIO_INTEN_BYTE(lgpio, pin));
	return 0;
}

static int ls2_set_gpio_inten(struct ls2_gpio_chip *lgpio, unsigned int pin, int enable)
{
	uint32_t u;
